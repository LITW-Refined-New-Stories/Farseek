...
/** A [[BlockSetter]] backed by an array of blocks and an array of metadata for
  * a chunk. It is meant for use during chunk generation by a
  * [[net.minecraft.world.gen.ChunkProviderGenerate]] or other generator
  * implementation.
  * @author
  *   delvr
  */
class ChunkBlockArrayAccess(
    val worldProvider: WorldProvider,
    xChunk: Int,
    zChunk: Int,
    blocks: Array[Block],
    datas: Option[Array[Byte]] = None
) extends BlockSetter
    with BoundedBlockAccess {
  ...
  private def index(x: Int, y: Int, z: Int) =
    ((x & 15) * 16 + (z & 15)) * height + y
