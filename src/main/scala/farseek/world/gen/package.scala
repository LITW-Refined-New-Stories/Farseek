...
package object gen {
  ...
  def registerWorldGenerator(
      generator: IWorldGenerator,
      modGenerationWeight: Int
  ) {
    val newGenerator =
      if (
        tfcLoaded && generator
          .isInstanceOf[com.bioxx.tfc.WorldGen.Generators.WorldGenForests]
      ) new tfc.WorldGenForests
      else generator
    GameRegistry.registerWorldGenerator(newGenerator, modGenerationWeight)
  }

  /** Replacement method for GameRegistry.generateWorld() that sets
    * [[populatingExtras]] to `true` during mod-provided chunk population.
    */
  def generateWorld(
      xChunk: Int,
      zChunk: Int,
  ...
  def chunkRandom(xChunk: Int, zChunk: Int)(implicit
      w: WorldProvider
  ): Random = {
  ...
  def sizedBox(x: Int, z: Int, size: Int)(implicit
      w: IBlockAccess
  ): BoundingBox = sizedBox(x, z, size, size)

  def sizedBox(xMin: Int, zMin: Int, xSize: Int, zSize: Int)(implicit
      w: IBlockAccess
  ): BoundingBox =
  ...
  def worldHeightBox(xMin: Int, zMin: Int, xMax: Int, zMax: Int)(implicit
      w: IBlockAccess
  ): BoundingBox =
  ...
  implicit class BoundingBoxValue(val box: BoundingBox) extends AnyVal {
    ...
    def intersects(chunk: Chunk) =
      intersectsChunkAt(chunk.xPosition, chunk.zPosition)

    def intersectsChunkAt(xChunk: Int, zChunk: Int) = box.intersectsWith(
      xChunk * ChunkSize,
      zChunk * ChunkSize,
      xChunk * ChunkSize + iChunkMax,
      zChunk * ChunkSize + iChunkMax
    )
