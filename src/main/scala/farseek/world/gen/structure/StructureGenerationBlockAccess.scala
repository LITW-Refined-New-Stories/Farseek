...
class StructureGenerationBlockAccess(provider: StructureGenerationChunkProvider)
    extends ServerBlockAccess
    with ChunkAccess {
  ...
  def chunkAt(x: Int, z: Int) = {
    val xChunk = x >> 4
    val zChunk = z >> 4
    loadedChunks.getOrElseUpdate(
      (xChunk, zChunk),
      provider.generateChunk(xChunk, zChunk)
    )
  }
}
