...
/** Farseek implementation of world generation [[Structure]] generators, as an
  * alternative to vanilla
  * [[net.minecraft.world.gen.structure.MapGenStructure]]s.
  *
  * @migration(message
  ...
  *   delvr
  */
abstract class StructureGenerator[T <: Structure[_]](
    chunksRange: Int,
    dimensionId: Int = SurfaceDimensionId
) extends Logging {
  ...
  def onChunkGeneration(
    ...
    if (
      world.dimensionId == dimensionId && !invalidWorldTypes.contains(
        world.terrainType
      ) &&
  ...
  protected def generate(
    ...
    for (
      xStructureChunk <- xChunk - chunksRange to xChunk + chunksRange;
      ...
      if (!structures.contains(xStructureChunk, zStructureChunk)) {
        ...
        implicit val random =
          chunkRandom(xStructureChunk, zStructureChunk)(worldProvider)
        val bounds = worldHeightBox(
          (xStructureChunk - chunksRange) * ChunkSize,
          (zStructureChunk - chunksRange) * ChunkSize,
          (xStructureChunk + chunksRange) * ChunkSize + iChunkMax,
  ...
  protected def createStructure(
      bounds: BoundingBox
  )(implicit worldAccess: IBlockAccess, random: Random): Option[T]

  @SubscribeEvent def onPrePopulateChunk(event: PopulateChunkEvent.Pre) {
    if (event.world.dimensionId == this.dimensionId)
      build(event.chunkX, event.chunkZ)(
        event.world.asInstanceOf[WorldServer],
        event.rand
      )
  }

  protected def build(xChunk: Int, zChunk: Int)(implicit
      world: WorldServer,
      random: Random
  ) {
    val area = new PopulatingArea(xChunk, zChunk, world)
    structures.foreach {
      case (_, Some(structure)) if structure.intersectsWith(area) =>
        structure.build(area, random)
      case _ =>
    }
  }
