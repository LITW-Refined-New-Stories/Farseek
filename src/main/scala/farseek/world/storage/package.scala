...
package object storage {
  ...
  /** Method patch for SaveHandler.readData that allows Farseek mods to warn
    * about loading existing worlds created without them.
    */
  def readData(
      container: WorldAccessContainer,
      handler: SaveHandler,
      worldInfo: WorldInfo,
      properties: java.util.Map[String, NBTBase],
      tags: NBTTagCompound
  ): Unit = {
    val abort = container.isInstanceOf[FMLContainer] && {
      val modTags = tags.getTagList("ModList", 10)
      val savedModIds = (for (i <- 0 until modTags.tagCount)
        yield modTags.getCompoundTagAt(i).getString("ModId")).toSet
      val missingFarseekMods =
        Loader.instance.getModList.map(_.getMod).collect {
    ...
    if (abort) StartupQuery.abort()
    else container.readData(handler, worldInfo, properties, tags)
  }
}
