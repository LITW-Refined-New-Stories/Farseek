...
object Direction {
  ...
  val CornerDirections3d =
    OrdinalDirections.map(Up + _) ++ OrdinalDirections.map(Down + _)
  val AllDirections3d = CardinalDirections3d ++ CornerDirections3d

  def neighbor(x: Int, z: Int, d: Direction): XZ = (x + d.x, z + d.z)
  def neighbor(x: Int, y: Int, z: Int, d: Direction): XYZ =
    (x + d.x, y + d.y, z + d.z)

  def neighbors(x: Int, z: Int): Seq[XZ] =
    CardinalDirections.map(neighbor(x, z, _))
  def cornerNeighbors(x: Int, z: Int): Seq[XZ] =
    OrdinalDirections.map(neighbor(x, z, _))
  def allNeighbors(x: Int, z: Int): Seq[XZ] =
    CompassDirections.map(neighbor(x, z, _))

  def neighbors(x: Int, y: Int, z: Int): Seq[XYZ] =
    CardinalDirections.map(neighbor(x, y, z, _))
  def cornerNeighbors(x: Int, y: Int, z: Int): Seq[XYZ] =
    OrdinalDirections.map(neighbor(x, y, z, _))
  def allNeighbors(x: Int, y: Int, z: Int): Seq[XYZ] =
    CompassDirections.map(neighbor(x, y, z, _))
}
