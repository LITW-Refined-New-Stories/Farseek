...
/** An extension of [[IBlockAccess]] with some default implementations, and
  * methods to access the [[WorldProvider]] and validate coordinates.
  * @author
  *   delvr
  */
trait BlockAccess extends IBlockAccess {
  ...
  def isAirBlock(x: Int, y: Int, z: Int) =
    getBlock(x, y, z).isAir(this, x, y, z)
  ...
  def isSideSolid(
      ...
      side: ForgeDirection,
      default: Boolean
  ) = getBlock(x, y, z).isSideSolid(this, x, y, z, side)

  def isBlockProvidingPowerTo(x: Int, y: Int, z: Int, direction: Int) =
    getBlock(x, y, z).isProvidingStrongPower(this, x, y, z, direction)
}

/** A server-side [[BlockAccess]] where all client-only methods will throw an
...
trait ServerBlockAccess extends BlockAccess {
  ...
  @SideOnly(CLIENT) def getLightBrightnessForSkyBlocks(
      x: Int,
      y: Int,
      z: Int,
      lightValue: Int
...
/** A [[BlockAccess]] where getters are delegated to the chunk returned by
  * `chunkAt()`.
  * @author
  *   delvr
  */
trait ChunkAccess extends BlockAccess {
  ...
  def getBlock(x: Int, y: Int, z: Int) =
    chunkAt(x, z).getBlock(x & 15, y, z & 15)

  def getBlockMetadata(x: Int, y: Int, z: Int) =
    chunkAt(x, z).getBlockMetadata(x & 15, y, z & 15)

  def getTileEntity(x: Int, y: Int, z: Int) =
    chunkAt(x, z).func_150806_e(x & 15, y, z & 15)
}

/** A [[BlockAccess]] that wraps another `BlockAccess` and delegates all methods
  * except `validAt()` to it.
  * @author
  *   delvr
  */
trait WrappedBlockAccess extends BlockAccess {
  ...
  def getBlockMetadata(x: Int, y: Int, z: Int) =
    wrapped.getBlockMetadata(x, y, z)
