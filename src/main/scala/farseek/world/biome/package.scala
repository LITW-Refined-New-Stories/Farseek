...
package object biome {
  ...
  implicit class BiomeValue(val biome: BiomeGenBase) extends AnyVal {
    ...
    /** Returns the base biome for a [[BiomeGenMutated]], or the biome itself
      * otherwise.
      */
    ...
    def fillerBlockAndData: BlockAndData =
      (biome.fillerBlock, biome.field_76754_C)
  }

  /** Returns the vanilla grass block, or the appropriate TFC grass block if TFC
    * is loaded.
    */
  def grassBlockFor(x: Int, y: Int, z: Int)(implicit
      bac: IBlockAccess,
      cs: CoordinateSystem = AbsoluteCoordinates
  ): BlockAndData =
    if (tfcLoaded)
      tfcGroundBlockAndData(
        x,
        y,
        z,
        layerData => getTypeForGrassWithRain(layerData, tfcRainfall(x, y, z))
      )
    else grass

  /** Returns the vanilla dirt block, or the appropriate TFC dirt block if TFC
    * is loaded.
    */
  def dirtBlockFor(x: Int, y: Int, z: Int)(implicit
      bac: IBlockAccess,
      cs: CoordinateSystem = AbsoluteCoordinates
  ): BlockAndData =
  ...
  /** Returns the vanilla biome-specific sand block, or the appropriate TFC sand
    * block if TFC is loaded.
    */
  def sandBlockFor(x: Int, y: Int, z: Int)(implicit
      bac: IBlockAccess,
      cs: CoordinateSystem = AbsoluteCoordinates
  ): BlockAndData =
  ...
  /** Returns the vanilla gravel block, or the appropriate TFC gravel block if
    * TFC is loaded.
    */
  def gravelBlockFor(x: Int, y: Int, z: Int)(implicit
      bac: IBlockAccess,
      cs: CoordinateSystem = AbsoluteCoordinates
  ): BlockAndData =
  ...
  /** Returns the vanilla biome-specific rock block (always stone if the block
    * above is stone), or the appropriate TFC rock block if TFC is loaded.
    */
  def rockBlockFor(x: Int, y: Int, z: Int)(implicit
      bac: IBlockAccess,
      cs: CoordinateSystem = AbsoluteCoordinates
  ): BlockAndData = {
  ...
  private def tfcGroundBlockAndData(x: Int, y: Int, z: Int, f: Int => Block)(
      implicit
      bac: IBlockAccess,
      cs: CoordinateSystem = AbsoluteCoordinates
  ): (Block, Int) = {
  ...
  private def tfcLayer(x: Int, y: Int, z: Int)(implicit
      bac: IBlockAccess,
      cs: CoordinateSystem = AbsoluteCoordinates
  ) = {
  ...
  def tfcRainfall(x: Int, y: Int, z: Int)(implicit
      bac: IBlockAccess,
      cs: CoordinateSystem = AbsoluteCoordinates
  ) = {
    val w = bac.worldProvider.worldObj
  ...
  def tfcTemperature(x: Int, z: Int)(implicit
      bac: IBlockAccess,
      cs: CoordinateSystem = AbsoluteCoordinates
  ) = {
    val w = bac.worldProvider.worldObj
