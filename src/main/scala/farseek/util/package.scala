...
package object util {
  ...
  def wordWrap(text: Seq[String], columns: Int): Seq[String] =
    text
      .flatMap(_.split('\n'))
      .flatMap(line =>
        s"(.{1,$columns})($$|\\s)".r.findAllMatchIn(line).map(_.group(1))
      )
  ...
  def randomElement[T](seq: Array[T])(implicit random: Random): T = seq(
    random.nextInt(seq.length)
  )

  def randomElementOption[T](seq: Array[T])(implicit
      random: Random
  ): Option[T] = if (seq.nonEmpty) Some(randomElement(seq)) else None
  ...
  def copyOf[T: ClassTag](src: Array[T]): Array[T] =
    copyTo(src, new Array[T](src.length))

  def between(start: Int, end: Int): Range =
    if (start <= end) start to end else start to end by -1

  def hypotenuse(a: Double, b: Double): Double = sqrt(a * a + b * b)

  def hypotenuse(a: Double, b: Double, c: Double): Double = sqrt(
    a * a + b * b + c * c
  )

  def distance(x1: Int, z1: Int, x2: Int, z2: Int): Double =
    hypotenuse(x2 - x1, z2 - z1)

  def distance(x1: Int, y1: Int, z1: Int, x2: Int, y2: Int, z2: Int): Double =
    hypotenuse(x2 - x1, y2 - y1, z2 - z1)

  def fillSeq[T](times: Int, element: T): Seq[T] =
    Seq.fill(times)(element) // ensures element is computed only once
  ...
  implicit class XyzValue(val xyz: XYZ) extends AnyVal {
    ...
    def distanceTo(dest: XYZ): Double =
      distance(x, y, z, dest.x, dest.y, dest.z)
    def debug = s"tp $x $y $z"
  }

  /** Value class for [[Vec3]] 3D vectors with utility methods.
    * @author
    *   Vectron himself, in the First Age of Vectron.
    */
  implicit class Vectron(val vec: Vec3) extends AnyVal {
    def +(other: Vec3) = createVectorHelper(
      vec.xCoord + other.xCoord,
      vec.yCoord + other.yCoord,
      vec.zCoord + other.zCoord
    )
    def -(other: Vec3) = createVectorHelper(
      vec.xCoord - other.xCoord,
      vec.yCoord - other.yCoord,
