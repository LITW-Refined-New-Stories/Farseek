...
/** A [[ConfigElement]] setting with help text and load-time value parsing and
  * defaulting.
  * @author
  *   delvr
  */
abstract class Setting[T](
    ...
    rawHelp: String,
    defaultValue: () => T
) extends ConfigElement(Some(category), name) {
    ...
    case Some(props) =>
      ...
      value =
        try parse(prop)
          ...
          case NonFatal(_) =>
            warn(
              s"Value $prop is not valid for setting $id; substituting default value $defaultValue"
            )
            defaultValue()
        }
  ...
  def save(writer: PrintWriter, columns: Int) {
    wordWrap(help, columns - commentPrefix.length).foreach(line =>
      writer.println(commentPrefix + line)
    )
