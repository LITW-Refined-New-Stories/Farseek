...
class NumericSetting(
  ...
  require(
    max > min && step > 0 && step <= max - min,
    s"Invalid min/max/step/ for setting $this"
  )
  ...
  protected def valuesHelp = Seq(
    s"Range: ${doubleToString(min)} to ${doubleToString(max)} (default: ${doubleToString(defaultValue())})"
  )
