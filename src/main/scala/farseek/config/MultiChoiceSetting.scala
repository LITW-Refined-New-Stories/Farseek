...
class MultiChoiceSetting(
  ...
  require(
    values.map(_.toString.toLowerCase).toSet.size == values.size,
    "Duplicate values for setting $this"
  )

  protected def parse(s: String) =
    values.find(_.toString.equalsIgnoreCase(s)).get

  protected lazy val valuesHelp =
    ("Valid values: " + values
      .map(v => v + (if (v == defaultValue) " (default)" else ""))
      .mkString(", ") + '.') +:
...
case class CustomChoice(name: String, help: () => String)
    extends ChoiceValue(name)
