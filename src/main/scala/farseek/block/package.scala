...
/** Utility functions and value classes related to [[Block]]s and their
  * metadata. Note: to ensure that all custom blocks have been loaded by mods,
  * do not access this object until mod initialization is complete.
  * @author
  *   delvr
  */
package object block {
  ...
  lazy val allBlocks: Set[Block] =
    blockRegistry.registryObjects.values.toSet[Block]

  /** Blocks from [[allBlocks]] for which the best harvesting tool is the
    * shovel.
    */
  lazy val granularBlocks: Set[Block] = {
    Class.forName(
      classOf[ForgeHooks].getName
    ) // Ensure static initializer execution
    allBlocks.filter(block =>
      block.isSolid && block.getHarvestTool(0) == "shovel"
    )
  }

  /** Blocks from [[granularBlocks]] that have ground or grass as material. (For
    * TFC, filters with [[TFC_Core.isSoil]] instead).
    */
  lazy val soilBlocks: Set[Block] = granularBlocks.filter(block =>
    if (tfcLoaded) isSoil(block)
    else
      block.getMaterial == Material.ground || block.getMaterial == Material.grass
  )

  /** Blocks from [[granularBlocks]] that have clay or sand (this includes
    * gravel) as material. (For TFC, also includes those for which
    * [[TFC_Core.isGravel]] is `true`).
    */
  lazy val sedimentBlocks: Set[Block] = granularBlocks.filter(block =>
    block.getMaterial == Material.sand || block.getMaterial == Material.clay || (tfcLoaded && isGravel(
      block
    ))
  )

  /** Blocks from the Forge ore dictionary registered as "stone" or "standstone"
    * or any string starting with "ore", as well as the specific blocks hardened
    * clay, stained hardened clay, netherrack, End stone and bedrock.
    */
  lazy val naturalStoneBlocks: Set[Block] =
    dictionaryBlocks("stone") ++ dictionaryBlocks(
      "sandstone"
    ) ++ dictionaryBlocks(_.startsWith("ore")) +
  ...
  def dictionaryBlocks(key: String): Set[Block] =
    dictionaryBlocksAndData(key).map(_.block)

  /** Returns the Forge ore dictionary blocks matching filter `keyFilter`. */
  def dictionaryBlocks(keyFilter: String => Boolean): Set[Block] =
    dictionaryBlocksAndData(keyFilter).map(_.block)

  /** Returns the Forge ore dictionary blocks registered with the name `key`,
    * with metadata.
    */
  def dictionaryBlocksAndData(key: String): Set[BlockAndData] =
    getOres(key).flatMap(itemStackBlockAndData).toSet

  /** Returns the Forge ore dictionary blocks matching filter `keyFilter`, with
    * metadata.
    */
  def dictionaryBlocksAndData(keyFilter: String => Boolean): Set[BlockAndData] =
    getOreNames
      .filter(name => name != null && keyFilter(name))
      .flatMap(dictionaryBlocksAndData)
      .toSet
  ...
  implicit class BlockValue(val block: Block) extends AnyVal {
    ...
    def isDiscreteObject =
      !(block.displayOnCreativeTab == tabBlock || (tfcLoaded && block.displayOnCreativeTab == TFC_BUILDING))
  }

  /** Returns a human-readable localized metadata-independent name for `block`
    * on a best-effort basis. (Some blocks only have data-specific names such as
    * colored wool or clay; for these we fall back on the ore dictionary or
    * unlocalized name.)
    */
  def displayName(block: Block): String = {
    if (block == stained_hardened_clay)
      ...
      if (displayName != name + ".name")
        ...
        getOreIDs(new ItemStack(block)).headOption match {
          case Some(id) =>
            val dictionaryKey = getOreName(id)
            val dictionaryName = s"tile.$dictionaryKey.name"
            val dictionaryDisplayName =
              StatCollector.translateToLocal(dictionaryName)
